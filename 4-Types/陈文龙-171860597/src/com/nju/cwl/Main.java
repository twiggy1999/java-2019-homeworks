...
//				tip.setStyle(Color.BLACK);
	...
	public void addHuman() throws ClassNotFoundException, InstantiationException, IllegalAccessException, NoSuchMethodException, SecurityException, IllegalArgumentException, InvocationTargetException
	{
		int i;
		for (i = 0; i < 7; ++i)
		{
			...
			/**
			*如果使用newInstance（）的话，一方面这个方面是过时的，另一方面要调用这个方法必须是存在默认构造函数
			*使用指定的构造函数来调用方法更合适
			*/
	...
	public void addMonster() throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException, ClassNotFoundException
	{
		int index = 0;
