...
public class Map {
    ...
    private OrganismGenerator generator=new OrganismGenerator();
    ...
    public Map(int maxlength,int maxwidth)
    {
        ...
        for(int i =0;i<len;i++)
        {
            for(int j=0;j<width;j++) all[i][j]=generator.next("Land");
        }
    ...
    private void initArray()
    {
        ...
        array.add(formationGenerator.next("CraneWingFormation"));
        array.add(formationGenerator.next("WildGooseFormation"));
        array.add(formationGenerator.next("YoKeFormation"));
        array.add(formationGenerator.next("FishScaleFormation"));
        array.add(formationGenerator.next("FangWeiFormation"));
        array.add(formationGenerator.next("HaoYueFormation"));
        array.add(formationGenerator.next("FrontalFormation"));

    }

    public void selectFormation(int index)
    {
        if(index>=0) addFormation(array.get(index));
        else System.out.println("请输入合法的索引！");
    }
