...
class CalabashBrother extends Soldiers<CalabashBoy> {
  ...
  public static class CalabashBoy extends Unit {
    private Color color;
    private String name;
    private char symbol;

    CalabashBoy(Color color, Position position) {
    ...
    public void showPosition() {
      System.out.printf("%c :%s\n", this.symbol, this.getPosition().toString());
    }
  }

  private static final int CALABASHBOY_NUM = 7;
  ...
  CalabashBrother() {
    ...
    for (int i = 0; i < CALABASHBOY_NUM; i++) {
      this.soldiers.addElement(new CalabashBoy(Color.values()[i], new Position(i + 1, 0)));
      this.number++;
    }
  }
}
