...
public class Huluwa implements IBeing, IFighter {
    private int rank = -1;
    ...
    public Huluwa(String n, int r, int fa) {
        name = n;
        rank = r;
        fightAbility = fa;
    }
    ...
    public void fightSingleStep() {
        ...
        if (Involved.a.get(no).getCombatComp() != 0//前方不是中立方
                ...
                && Involved.a.get(no).getAliveState()) {//且还活着
            if (fightWith(no) == -1) {
                alive = false;
                //BattleField.setField(x1, y1, 0);
                //Involved.printBf();
        ...
        if (!Involved.a.get(no).getAliveState() && y1 < 7) {//在限界内 前方为空 死人 同一特点 都是死的
            ...
            no = BattleField.getNo(x1, y1 + 2);
            if (!Involved.a.get(no).getAliveState() && y1 < 6) {
                BattleField.setField(x1, y1 + 2, rank);//swap
                BattleField.setField(x1, y1 + 1, no);
            }
            //Involved.printBf();
            return;
        }
