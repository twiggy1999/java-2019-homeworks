...
public class Processor {
    ...
    public void init_grandpa(){
        ...
        block[0][0].init_(new Grandpa(),0,0,Identity.GRANDPA,Picture.GR,Color_of_Calabash.NO);
    ...
    public void init_calabash() {
        for(int i=1;i<8;i++)
        {
            switch (i) {
                case 1:
                    block[1][0].init_(new Calabash(),0,1,Identity.CALABASH,Picture.CA1,Color_of_Calabash.RED);
                    break;
                case 2:
                    block[2][0].init_(new Calabash(),0,2,Identity.CALABASH,Picture.CA2,Color_of_Calabash.ORANGE);
                    break;
                case 3:
                    block[3][0].init_(new Calabash(),0,3,Identity.CALABASH,Picture.CA3,Color_of_Calabash.YELLOW);
                    break;
                case 4:
                    block[4][0].init_(new Calabash(),0,4,Identity.CALABASH,Picture.CA4,Color_of_Calabash.GREEN);
                    break;
                case 5:
                    block[5][0].init_(new Calabash(),0,5,Identity.CALABASH,Picture.CA5,Color_of_Calabash.CYAN);
                    break;
                case 6:
                    block[6][0].init_(new Calabash(),0,6,Identity.CALABASH,Picture.CA6,Color_of_Calabash.BLUE);
                    break;
                case 7:
                    block[7][0].init_(new Calabash(),0,7,Identity.CALABASH,Picture.CA7,Color_of_Calabash.PURPLE);
                    break;
                default:
                    break;
    ...
    public void init_pips(Eorm eormation) {
        ...
        for(;i<num_of_pips;i++)
        {
            int row=h.ret_row(i);
            int col=h.ret_col(i);
            block[row][col].init_(new Pips(),col,row,Identity.PIPS,Picture.PI,Color_of_Calabash.NO);
    ...
    {
        block[0][N-1].init_(new Snake(),N-1,0,Identity.SNAKE,Picture.SN,Color_of_Calabash.NO);
    }
    public void init_scorpion(){
        block[num_of_pips/2+1][N-1-num_of_pips/2].init_(new Scorpion(),num_of_pips/2-1,N-1-num_of_pips/2,Identity.SCORPION,Picture.SC,Color_of_Calabash.NO);}
